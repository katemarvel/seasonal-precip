...
if __name__ == "__main__":
    ...
    for fname in fnames:
        try:
            PET,VPD,RH =  PET_from_cmip(fname,temp_variable = "tasmax")
            writefname = fname.split("/")[-1].replace("xml","nc").replace("sfcWind","PET_tasmax")
            ...
            PET,VPD,RH =  PET_from_cmip(fname,temp_variable = "tasmin")
            writefname = fname.split("/")[-1].replace("xml","nc").replace("sfcWind","PET_tasmin")
            fw = cdms.open(path+writefname,"w")
            fw.write(PET)
            fw.write(VPD)
            fw.write(RH)
