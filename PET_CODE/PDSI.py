...
def end_spell_P(U,Ze,nmonths,current_month,typ="wet"):
    ...
    nmonths=nmonths.astype(np.int64)
    if type(nmonths)==type(MV.array([])):
        nmonths=nmonths.asma()
...
def calc_PDSI(Z,BTthresh,calc_modified=True):
    ...
    for i in range(Z.shape[0])[1:]: #Loop over time
        ...
        for point in unique_points:
            ...
            possible_PDSI = [X1[F,lat,lon],X2[F,lat,lon]]
            the_month = len(F)-1
            last_month_pon=0
            ...
            while the_month >= 0:
                ...
                if possible_pdsi[the_month,pon] !=0:
                ...
                if ((the_month < len(F)-1) and (F[-1] < Z.shape[0])):
                    ...
                    for newj in range(len(F)):
                        if np.abs(X[F[newj]])>1:
                            if np.sign(X[F[newj]]) == np.sign(X[F[newj]-1]):
